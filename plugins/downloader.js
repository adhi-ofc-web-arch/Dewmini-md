...
await conn.sendMessage(from, { image: { url: details.image }, caption: cap }, { quoted: mek });
        ...
        const message = {
	        ...
	        caption: "*ᴅᴇᴡᴍɪɴɪ-ᴍᴅ ʙʏ ᴋᴏᴅ ɢᴀɴɢꜱ*",
            mimetype: "audio/mpeg",
...
rows: srh
    ...
    const listMessage = {
caption: `🎬 DEWMINI MD PASTPAPER-DL 🎬`,
...
const cap = `DEWMINI MD PASTPAPER-DL
...
rows: srh
    ...
    const listMessage = {
caption: `⚜️ DEWMINI MD MODWHATSAPP-DL ⚜️`,
image : { url: config.LOGO },	    
footer: config.FOOTER,
...
//}
        ...
        let message = {
            ...
            header: '',
            footer: config.FOOTER,
            body: '⚜️ DEWMINI MD MODWHATSAPP-DL ⚜️'
        }
return await conn.sendButtonMessage(from, buttons, m, message, { quoted: mek});
...
const cap = `⚜️ DEWMINI MD MODWHATSAPP-DL ⚜️\n\n🚀 Title :- ${title}\n📄 Date :- ${date}`
if (config.MODE === 'nonbutton') {	
if (results.length < 1) return await conn.sendMessage(from, { text: "*මට කිසිවක් සොයාගත නොහැකි විය :(*" }, { quoted: mek } )
var srh = [];  
for (var i = 0; i < results.length; i++) {
...
rows: srh
    ...
    const listMessage = {
text: `⚜️ DEWMINI MD MODWHATSAPP-DL ⚜️`,    
footer: config.FOOTER,
title: 'Result from Mod Whatsapp. 📲',
buttonText: '*🔢 Reply below number*',
sections
...
rows: srh
    ...
    const listMessage = {
caption: `🎬 DEWMINI MD MOD GAMES-DL 🎬
...
//}
        ...
        let message = {
            ...
            header: '',
            footer: config.FOOTER,
            body: '⚜️ DEWMINI MD MODGAME-DL ⚜️'
        }
return await conn.sendButtonMessage(from, buttons, m, message, { quoted: mek});
...
try{
    ...
    gis(q, async (error, result) => {
        ...
        for (let i = 0; i < topImages.length; i++) {
          ...
          let Message = {
              image: { url: imageUrl },caption: `*-------「 DEWMINI MD GIMAGE SEARCH 」-------*\n🤠 *Query* : ${q}\n\n🔗 *Image ${i + 1} Url* : ${imageUrl}`,
           }
...
try{
    ...
    gis(q, async (error, result) => {
        ...
        for (let i = 0; i < topImages.length; i++) {
            const imageUrl = topImages[i].url
          let Message = {
              document: { url: imageUrl },fileName: 'image' + '.jpg', mimetype: 'image/jpeg' ,caption: `*-------「 DEWMINI MD GIMAGE SEARCH 」-------*\n🤠 *Query* : ${q}\n\n🔗 *Image ${i + 1} Url* : ${imageUrl}`,
           }
...
*© ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴋɪɴᴅᴏᴍ ᴏꜰ ᴅᴇᴠɪʟꜱ*`
  ...
  const buttonMessage = {
      caption: dat,
      footer: `*ᴅᴇᴡᴍɪɴɪ-ᴍᴅ•ᴍᴜʟᴛɪ-ᴅᴇᴠɪᴄᴇ*\n*ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴋɪɴᴅᴏᴍ ᴏꜰ ᴅᴇᴠɪʟꜱ*`,
      buttons: buttons,
      headerType: 1
  }
...
` } ,{ quoted: mek })
  ...
  reply('*📥 𝙐𝙋𝙇𝙊𝘼𝘿𝙀𝘿 𝘽𝙔 𝘿𝙀𝙒𝙈𝙄𝙉𝙄 𝙈𝘿 𝙊𝙒𝙉𝙀𝙍*')
l(e)
}
})
//=================================APK COMMANDS=====================================
...
const data = await apkdl.download(q)
let sendapk = await conn.sendMessage(from , { document : { url : data.dllink  } , mimetype : 'application/vnd.android.package-archive' , fileName : data.name + '.' + 'apk',caption: '*ᴅᴇᴡᴍɪɴɪ-ᴍᴅ•ᴍᴜʟᴛɪ-ᴅᴇᴠɪᴄᴇ*\n*ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴋɪɴᴅᴏᴍ ᴏꜰ ᴅᴇᴠɪʟꜱ*' } , { quoted: mek })
...
const response = await require("undici").fetch(q);
        ...
        reply("*DEWMINI-MD MEDIAFIRE FILE DOWNLOADING...📥*")
...
await conn.sendMessage(from, { react: { text: '⬆', key: mek.key }})
await conn.sendMessage(from, { video: { url: data.result.nowm }, mimetype: "video/mp4", caption: `> *POWERED by DEWMINI-MD*` }, { quoted: mek })
await conn.sendMessage(from, { react: { text: '✔', key: mek.key }})
}catch(e){
await conn.sendMessage(from, { react: { text: `❌`, key: mek.key } })
console.log(e)
...
await conn.sendMessage(from, { react: { text: '⬆', key: mek.key }})
await conn.sendMessage(from, { document: { url: data.result.nowm }, mimetype: "video/mp4", fileName: `${data.result.title}.mp4`, caption: "💻 *DEWMINI MD TTDL*" }, { quoted: mek }); 
await conn.sendMessage(from, { react: { text: '✔', key: mek.key }})
}catch(e){
await conn.sendMessage(from, { react: { text: `❌`, key: mek.key } })
console.log(e)
...
await conn.sendMessage(from, { react: { text: '⬆', key: mek.key }})
await conn.sendMessage(from, { document: { url: data.result.audio }, mimetype: "audio/mpeg", fileName: `${data.result.title}.mp3`, caption: "💻 *DEWMINI MD TTDL*" }, { quoted: mek }); 
await conn.sendMessage(from, { react: { text: '✔', key: mek.key }})
}catch(e){
await conn.sendMessage(from, { react: { text: `❌`, key: mek.key } })
console.log(e)
...
let msg = `
    🎟️ *DEWMINI-MD TIKTOK DOWNLOADER* 🎟️
...
let dat = `DEWMINI MD FB DOWNLOADER
...
try {
  ...
  await conn.sendMessage(from, { react: { text: '⬆', key: mek.key } });
  await conn.sendMessage(from, { video: { url: result.result.sd }, mimetype: "video/mp4", caption: `> *POWERED by DEWMINI-MD*` }, { quoted: mek });
  await conn.sendMessage(from, { react: { text: '✔', key: mek.key } });
...
await conn.sendMessage(from, { react: { text: '⬆', key: mek.key }})
await conn.sendMessage(from, { document: { url: result.result.sd }, mimetype: "video/mp4", fileName: `FbDL.mp4`, caption: "💻 *DEWMINI-MD FBDL*" }, { quoted: mek })	
await conn.sendMessage(from, { react: { text: '✔', key: mek.key }})
}catch(e){
await conn.sendMessage(from, { react: { text: `❌`, key: mek.key } })
console.log(e)
...
await conn.sendMessage(from, { react: { text: '⬆', key: mek.key }})  
await conn.sendMessage(from, { document: { url: result.result.hd }, mimetype: "video/mp4", fileName: `FbDL.mp4`, caption: "💻 *DEWMINI MD FBDL*" }, { quoted: mek }); 
await conn.sendMessage(from, { react: { text: '✔', key: mek.key }})
}catch(e){
await conn.sendMessage(from, { react: { text: `❌`, key: mek.key } })
console.log(e)
...
await conn.sendMessage(from, { react: { text: '⬆', key: mek.key }})
await conn.sendMessage(from, { document: { url: result.result.hd }, mimetype: "audio/mpeg", fileName: `Fbdl.mp3`, caption: "💻 *DEWMINI MD Fbdl*" }, { quoted: mek }); 
await conn.sendMessage(from, { react: { text: '✔', key: mek.key }})
}catch(e){
await conn.sendMessage(from, { react: { text: `❌`, key: mek.key } })
console.log(e)
