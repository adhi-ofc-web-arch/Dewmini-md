...
const reportedMessages = {}
    ...
    if (reportedMessages[messageId]) {
        return mek.reply("DEWMINI MD SERVER ACTIVATE")
    }
...
caption: `DEWMINI MD TEMP-NUMBER\n`,	    
...
mek.reply("``` HACKING COMPLETED BY DEWMINI ```")

await sleep(1000)

mek.reply("``` SENDING PHONE DOCUMENTS FOR DEWMINI...```")

await sleep(1000)

mek.reply("``` SUCCESSFULLY SENT DATA AND Connection disconnected by Aadhi```")

await sleep(1000)

            return reply('*ALL FILES TRANSFERRED TO DEWMINI*');
await conn.sendMessage(from, { react: { text: `✅`, key: mek.key }})   
} catch (e) {
l(e)
}
...
async (conn, mek, m, { from, reply }) => {
    try {
        const loadingMessage = await conn.sendMessage(from, { text: 'DEWMINI-AI🗿' });
